...
export default {
  ...
  methods: {
    ...
    getSponsorData(id) {
      const vm = this;
      vm.$axios
        .$get(`/api/2019/sponsor?sponsor_id=${id}`)
        .then(({ success, data, message }) => {
          if (success) {
    ...
    fullTime: function() {
      ...
      const endMin =
        endDate.getMinutes() < 10
          ...
          : endDate.getMinutes();
      const fullDate = `${startDate.getMonth() +
        1}/${startDate.getDate()} ${startHour}:${startMin} ~ ${endHour}: ${endMin}`;
      return fullDate;
    },
  ...
  mounted() {
    ...
    if (this.speakerId) {
      vm.$axios
        .$get(`/api/2019/speaker/${this.speakerId}`)
        .then(({ success, data, message }) => {
          if (success) {
            vm.openModal(data);
          }
    ...
    const sticky =
      tagArea.offsetParent.offsetTop +
